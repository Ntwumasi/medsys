...
export function validateVitalSign(
  ...
  const rangesWithCritical = ranges as any;
  if (rangesWithCritical.criticalMin !== undefined && rangesWithCritical.criticalMax !== undefined) {
    if (value < rangesWithCritical.criticalMin || value > rangesWithCritical.criticalMax) {
      return {
        isValid: true,
        isCritical: true,
        message: `Warning: Value is outside normal range (${rangesWithCritical.criticalMin}-${rangesWithCritical.criticalMax} ${ranges.unit})`,
        severity: 'critical',
      };
    }
  }
