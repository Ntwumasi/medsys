...
const ProgressTracker: React.FC<ProgressTrackerProps> = ({
  ...
  if (compact) {
    return (
      <div className="bg-gradient-to-br from-gray-50 via-primary-50 to-secondary-50 p-3 rounded-xl border border-primary-200 shadow-sm">
        <div className="flex items-center justify-between mb-2">
          <div className="text-xs font-bold text-gray-500 uppercase tracking-wider">Journey Progress</div>
          <div className="text-xl font-black bg-gradient-to-r from-primary-600 to-secondary-600 bg-clip-text text-transparent">
            {progress}%
          </div>
        </div>
        ...
        <div className="relative h-2 bg-gray-200 rounded-full overflow-hidden shadow-inner">
          <div
            className="absolute top-0 left-0 h-full bg-gradient-to-r from-primary-500 via-secondary-500 to-secondary-500 rounded-full transition-all duration-1000 ease-out"
          ...
          <div className="mt-2 flex gap-1 flex-wrap">
            {labOrders.length > 0 && (
              <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                labOrders.every(o => o.status === 'completed') ? 'bg-success-100 text-success-800' :
                labOrders.some(o => o.status === 'in_progress') ? 'bg-primary-100 text-primary-800' :
            ...
            {pharmacyOrders.length > 0 && (
              <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                pharmacyOrders.every(o => o.status === 'completed') ? 'bg-success-100 text-success-800' :
                pharmacyOrders.some(o => o.status === 'in_progress') ? 'bg-primary-100 text-primary-800' :
                'bg-warning-100 text-warning-800'
              }`}>
            ...
            {imagingOrders.length > 0 && (
              <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                imagingOrders.every(o => o.status === 'completed') ? 'bg-success-100 text-success-800' :
                imagingOrders.some(o => o.status === 'in_progress') ? 'bg-primary-100 text-primary-800' :
                'bg-warning-100 text-warning-800'
              }`}>
          ...
          {labOrders.length > 0 && (
            <div className={`px-3 py-1.5 rounded-lg font-semibold text-xs flex items-center gap-2 transition-all ${
              labOrders.every(o => o.status === 'completed')
                ...
                : 'bg-warning-100 text-warning-800 border border-amber-300'
            }`}>
