...
const ReceptionistDashboard: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<'queue' | 'checkin' | 'new-patient' | 'appointments' | 'invoices'>('queue');
  ...
  // Load invoices when view changes to invoices or filters change
  useEffect(() => {
    if (activeView === 'invoices') {
      loadInvoices();
    }
  }, [activeView, invoicesStatusFilter]);
      ...
      {error && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 xl:gap-6 mb-8">
          ...
          <button
            ...
            className={`bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-all cursor-pointer border-2 ${
              activeView === 'invoices' ? 'border-warning-500' : 'border-transparent'
            }`}
          >
            <div className="flex items-center">
